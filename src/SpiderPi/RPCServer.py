...
# Remote call api，framework jsonrpc
# Mainly used for mobile and computer client call
    ...
    while True:
        if not unload and stand_up_on:
            try:
                ...
                angle_x = int(math.degrees(math.atan2(accel_date['x'], accel_date['z'])))  # Convert that to an angle value
                if abs(angle_x) > 130:
                    count1 += 1
                else:
                    count1 = 0
                if abs(angle_x) < 50:
                    ...
                    count2 = 0
                time.sleep(0.1)
                if count1 >= 3:  # Tilt back for a while and get up
                    count1 = 0
                    if not control_lock:
                        control_lock = True
                        AGC.runAction('stand_flip')
                elif count2 >= 3:  # Tilt forward for a while and get up
                    count2 = 0
                    if control_lock:
                        control_lock = False
